...
async function backupContactFlow(accessKey, secretKey, region, instance, ddb, flowid, listBox){
	try{
		var jitter = 400;
   		var j = Math.random() * jitter;
   		sleep(j);
   		let cfDetails ;
   		try {
   			cfDetails = await getFlow(accessKey, secretKey, region, instance, flowid);	
   		} catch(e) {
   			console.log(e);
   	   		sleep(jitter * 3);
   	   		try {
...
const getFlow = (accessKey, secretKey, region, instance, flowid) =>{
	return new Promise((resolve,reject) =>{
		var p = setAWSConfig2(accessKey, secretKey, region);
...
const listContactFlows = (accessKey, secretKey, region, instanceId, maxResults, nextToken) =>{
	return new Promise((resolve,reject) =>{
	   var p = setAWSConfig2(accessKey, secretKey, region);
	   var params ;
	   if(nextToken)
		   params = {InstanceId : instanceId, MaxResults : maxResults, NextToken : nextToken};
	   else
		   params = {InstanceId : instanceId, MaxResults : maxResults};
       console.log(params);
...
async function getContactFlows(){
	...
	var limit = 100;
	try{
		var resp = await listContactFlows(dlgSourceAccessKey, dlgSourceSecretKey, dlgSourceRegion, dlgSourceInstance, limit);
		contactFlows = resp.ContactFlowSummaryList;
		
		while (resp.NextToken){
			sleep(500);
			resp = await listContactFlows(dlgSourceAccessKey, dlgSourceSecretKey, dlgSourceRegion, dlgSourceInstance, limit, resp.NextToken );
		    for (var i=0; i< resp.ContactFlowSummaryList.length; i++) {
		    	contactFlows.push(resp.ContactFlowSummaryList[i]);
		    }	
		}
...
async function getContactFlows2(){
	var limit = 100;
	try {
		var resp = await listContactFlows(dlgTargetAccessKey, dlgTargetSecretKey, dlgTargetRegion, dlgTargetInstance, limit);
		contactFlows2 = resp.ContactFlowSummaryList;
		
		while (resp.NextToken){
			sleep(500);
			resp = await listContactFlows(dlgTargetAccessKey, dlgTargetSecretKey, dlgTargetRegion, dlgTargetInstance, limit, resp.NextToken );
		    for (var i=0; i< resp.ContactFlowSummaryList.length; i++) {
		    	contactFlows2.push(resp.ContactFlowSummaryList[i]);
		    }	
		}
...
function setAWSConfig2(accessKey, secretKey, region){
		AWS.config.update({
	        accessKeyId: accessKey, secretAccessKey: secretKey, region: region, retryDelayOptions: {base: 600}, maxRetries : 3
	    });
		loadConnectAPIs();
}
